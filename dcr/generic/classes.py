...
class SerializeSQLAResult_:
    ...
    def format_columns(dict_row, key, sqla_result, full_date_fields):
        if not key.startswith('_') and key not in ['keys', 'index', 'count', 'metadata', 'query', 'query_class']:
            if type(getattr(sqla_result, key)) is datetime or type(getattr(sqla_result, key)) is date:
                if full_date_fields and key in full_date_fields:
                    dict_row[key] = getattr(sqla_result, key).strftime("%d-%m-%Y %I:%M %p")
                else:
                    dict_row[key] = getattr(sqla_result, key).strftime("%d-%m-%Y %I:%M %p")  # Include time
    ...
    @staticmethod
    def format_columns_date_(dict_row, key, sqla_result, full_date_fields):
        if not key.startswith('_') and key not in ['keys', 'index', 'count', 'metadata', 'query', 'query_class']:
            if type(getattr(sqla_result, key)) is datetime or type(getattr(sqla_result, key)) is date:
                if full_date_fields and key in full_date_fields:
                    dict_row[key] = getattr(sqla_result, key).strftime("%d-%m-%Y %I:%M %p")
    ...
    def serialize_one_(self, **kwargs):
        """
    ...
    # Added by Athira
    def serialize_date_(self, **kwargs):
        """
        Generating the dictionary variable by looping through the list of <class 'sqlalchemy.util._collections.result'> objects
        @return: List of serialized SQLA result.
        """
...
class GenerateReport:
    ...
    def generate(self):
        ...
        if self.is_log is False:
            ...
            # Check if self.list_var is empty
            if not self.list_var:
                # Handle the case where self.list_var is empty
                print("List is empty, no data to generate report.")
                return self

            # Dict keys will be the sheet's header.
            dict_keys = self.list_var[0].keys()
            keys = list(dict_keys)  # Simplified way to get keys
            sheet.append(keys)
