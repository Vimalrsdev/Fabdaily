...
from dcr.modules.models import DCR_DateWiseCollections
    ...
    if result is not None:
        for Daily_Collection in result:
            ...
            already_amount = db.session.query(DCR_DateWiseCollections).filter(
                DCR_DateWiseCollections.BranchCode == Daily_Collection['BranchCode'],
                DCR_DateWiseCollections.BillNo == bill_no,
        ...
        for collection in result:
            cash_to_be_collected = db.session.query(DCR_DateWiseCollections.PendingAmount).filter(
                ...
                DCR_DateWiseCollections.BillNo == collection['BillNo'],
                DCR_DateWiseCollections.IsDeleted == 0).one_or_none()
...
def egrn_wise_collection(result, store_id, branch_name, collection_amount):
    if result is not None:
        for Daily_Collection in result:
            ...
            already_amount = db.session.query(DCR_DateWiseCollections).filter(
                DCR_DateWiseCollections.BranchCode == Daily_Collection['BranchCode'],
                DCR_DateWiseCollections.EGRN == EGRN,
                DCR_DateWiseCollections.IsDeleted == 0).one_or_none()
            if already_amount is not None:
                bill_amount = Daily_Collection['BillAmount']
                if already_amount.TotalAmount < bill_amount:
